  ...
  "emotionBreakdown": { "calm": (angka), "anxious": (angka), "confident": (angka), "fearful": (angka) }
    ...
    const review = await db.weeklyReviewRecord.create({
      data: {
        ...
        processScore: typeof aiData.processScore === 'number' ? Math.round(aiData.processScore) : 50,
        ruleCompliance: typeof aiData.ruleCompliance === 'number' ? aiData.ruleCompliance : 50,
        ...
        playbookUsage: typeof aiData.playbookUsage === 'number' ? aiData.playbookUsage : 50,
        emotionBreakdown,
      },
    })
